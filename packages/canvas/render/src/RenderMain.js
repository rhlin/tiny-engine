...
import { h, provide, inject, nextTick, shallowReactive, reactive, ref, watch, watchEffect, defineComponent } from 'vue'
...
function useAccessorMap(withContext) {
  ...
  const generateAccessor = (type, accessor, property) => {
    const accessorFn = generateFunction(accessor[type].value, withContext ? withContext.context : context)
...
function useState(withContext, generateStateAccessors, schema) {
  // data
  ...
  const setState = (data, clear) => {
    ...
    Object.assign(state, parseData(data, {}, (withContext?.getContext || getContext)()) || {})
...
function useMethod(withContext) {
  const methods = {}
    ...
    Object.assign(
      ...
      Object.fromEntries(
        Object.keys(data).map((key) => {
          return [key, parseData(data[key], {}, (withContext?.setContext || getContext)())]
        })
      )
    )
    ;(withContext?.setContext || setContext)(methods)
  }
  return {
    methods,
    getMethods,
...
function useProps(generateAccessor) {
  const props = {}
...
const setPagecss = (css = '', pageId = '') => {
  const id = 'page-css' + pageId ? `-${pageId}` : ''
...
const { utils, getUtils, setUtils, updateUtils, deleteUtils, refreshKey } = useUtils()
const { bridge, setBridge, getBridge } = useBridge()
const { getDataSourceMap, setDataSourceMap } = useDataSourceMap()

export function useSchema(isolate = false) {
  const schema = reactive({})
  const withContext = isolate ? useWithContext() : null

  const { propsAccessorMap, generateAccessor, generateStateAccessors, clearSideEffect } = useAccessorMap(withContext)
  const { props, setProps, getProps, initProps } = useProps(generateAccessor)
  const { state, getState, deleteState, setState } = useState(withContext, generateStateAccessors, schema)
  const { setMethods, getMethods } = useMethod(withContext)
  ...
  const setSchema = async (data) => {
    ...
    ;(withContext?.setContext || setContext)(context, true)
    ...
    ;(withContext?.clearNodes || clearNodes)()
    await nextTick()
    setPagecss(data.css)
  ...
  const getNode = (id, parent) => (id ? (withContext?.getNode || getNodeById)(id, parent) : schema)
  return {
    schema,
    setSchema,
    getSchema,
    ...
    api: {
      ...
      ...{
        // 覆盖sharedApi
        getContext: withContext?.getContext || getContext,
        getRoot: withContext?.getRoot || getRoot,
        setCondition: withContext?.setCondition || setCondition
      }
    }
  }
}

export const CANVAS_API = Symbol('canvas-api')
export const WITH_CONTEXT = Symbol('with-context')
...
const { schema, setMethods, api: currentSchemaApi } = useSchema()
